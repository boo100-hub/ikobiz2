...
export const api = {
  ...
  async patch(path: string, data: unknown, auth = false) {
    const r = await fetch(API_BASE + path, {
      method: 'PATCH',
      headers: getHeaders(auth),
      body: JSON.stringify(data),
...
export type Shop = {
  ...
  category?: string; location_area?: string
  fulfillment_modes?: string; delivery_radius_km?: number
  delivery_fee?: number; operating_hours?: string
  payment_methods?: string; pickup_address?: string
  phone?: string
}
...
export type Product = {
  ...
  category?: string; attributes?: string
}
...
export type Order = {
  ...
  fulfillment_method?: string; delivery_area?: string
  delivery_address?: string; delivery_fee?: number
